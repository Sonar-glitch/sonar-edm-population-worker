//Create a completely clean eventValidation.js file
...
function safeLocation(location, venue) {
  const defaultCoords = [-79.3832, 43.6532]; // Toronto
...
function normalizeSourceName(source) {
  ...
  // Map various source formats to consistent names
  switch (normalized) {
    case 'ticketmaster':
    case 'tm':
      return 'ticketmaster';
...
function validateAndNormalizeEvent(event, source = 'unknown') {
  ...
  try {
    ...
    const genresData = safeGenres(
  event.genres || 
  [event.genre, event.subGenre].filter(Boolean) ||
  event.classifications?.map(c => c.genre?.name)
    ...
    return {
      id: safeString(event.sourceId || event.id, `${source}-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`),
      ...
      image: imagesData.length > 0 ? imagesData[0].url : '',
    ...
    console.warn(`Validation error for event ${event.id}:`, error.message);
    return null;
  }
}

/**
 * ENHANCED DEDUPLICATION: Sophisticated logic with time-awareness and quality scoring
 * Replaces the original simple deduplication by source and sourceId
 */
function mergeAndDeduplicateEvents(events) {
  ...
  for (const event of events) {
    ...
    const dateOnly = event.date ? new Date(event.date).toISOString().split('T')[0] : '';
    const normalizedVenue = event.venue?.name?.toLowerCase().trim();
    const timeComponent = event.startTime || event.date ? 
      new Date(event.date).getHours() + ':' + new Date(event.date).getMinutes() : 'no-time';
    
    // Create sophisticated key that preserves legitimate differences
    const key = `${normalizedName}-${dateOnly}-${normalizedVenue}-${timeComponent}`;
    
    if (!seen.has(key)) {
      seen.add(key);
      deduplicated.push(event);
    } else {
      // If duplicate found, keep the one with higher quality score
      const existingIndex = deduplicated.findIndex(e => {
        const existingNormalizedName = e.name?.toLowerCase().trim().replace(/[^\w\s]/g, '');
        const existingDateOnly = e.date ? new Date(e.date).toISOString().split('T')[0] : '';
        const existingNormalizedVenue = e.venue?.name?.toLowerCase().trim();
        const existingTimeComponent = e.startTime || e.date ? 
          new Date(e.date).getHours() + ':' + new Date(e.date).getMinutes() : 'no-time';
        const existingKey = `${existingNormalizedName}-${existingDateOnly}-${existingNormalizedVenue}-${existingTimeComponent}`;
        return existingKey === key;
      });
      
      if (existingIndex !== -1) {
        const existingQuality = calculateCompletenessScore(deduplicated[existingIndex]);
        const currentQuality = calculateCompletenessScore(event);
        
        if (currentQuality > existingQuality) {
          console.log(`🔄 Enhanced dedup: Replacing lower quality duplicate: ${event.name} (${currentQuality} > ${existingQuality})`);
          deduplicated[existingIndex] = event; // Replace with higher quality
        } else {
          console.log(`⚠️ Enhanced dedup: Keeping existing higher quality: ${event.name} (${existingQuality} >= ${currentQuality})`);
        }
      }
    }
  }
