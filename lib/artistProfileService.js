...
class ArtistProfileService {
  ...
  // Per-artist quick metrics cache (in-memory). Useful for short-term monitoring
  this.perArtistMetrics = {}; // { artistName: { source, success, confidence, tracksAnalyzed, timestamp } }

  // Allow disabling SoundCloud fallback via env var (default: disabled)
  this.soundCloudEnabled = process.env.SOUNDCLOUD_ENABLED === 'true';
  ...
  async generateArtistProfile(artistName, options = {}) {
    ...
    try {
      ...
      // Record per-artist metric for quick inspection (in-memory)
      try {
        this.perArtistMetrics[artistName] = {
          source: profile.source,
          success: !!(profile.confidence && profile.confidence > 0.3),
      ...
      // Persist per-artist metric to MongoDB if connected (non-blocking)
      try {
        if (process.env.MONGODB_URI) {
          // Lazy connect using mongodb native driver to avoid adding a mongoose dependency here
          const { MongoClient } = require('mongodb');
          MongoClient.connect(process.env.MONGODB_URI, { useUnifiedTopology: true }).then(async client => {
            try {
              const db = client.db();
              await db.collection('artist_profile_metrics').updateOne(
                { artistName },
                { $set: { artistName, source: profile.source, success: !!(profile.confidence && profile.confidence > 0.3), confidence: profile.confidence || 0, tracksAnalyzed: profile.tracksAnalyzed || 0, timestamp: new Date() } },
                { upsert: true }
              );
            } catch (e) {
              // ignore persistence failure
              console.warn('⚠️ Persisting per-artist metric failed:', e.message);
            } finally {
              try { await client.close(); } catch(_){}
            }
          }).catch(err => console.warn('⚠️ Mongo connect for metrics failed:', err.message));
        }
      } catch (e) {
        // non-fatal
      }
  ...
  async trySoundCloudReccoBeats(artistName, options) {
    ...
    // If SoundCloud is disabled globally, skip this path immediately
    if (!this.soundCloudEnabled) {
      result.errorCodes.push('SOUNDCLOUD_DISABLED');
      result.processingSteps.push('SoundCloud fallback disabled by configuration');
  ...
  // Expose per-artist quick metrics for monitoring
  getPerArtistMetrics() {
    return this.perArtistMetrics;
  }
