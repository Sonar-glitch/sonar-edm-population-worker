...
class RecommendationEnhancer {
  constructor() {
  // Allow forcing enhancement during one-off runs or dry-runs
  this.enabled = process.env.RECOMMENDATION_ENHANCEMENT_ENABLED === 'true' || process.env.FORCE_RECOMMENDATION_ENHANCEMENT === 'true';
    ...
    this.artistGenres = new Map([
      ...
      ['don diablo', { genre: 'future house', weight: 1.0 }],
      ...
      ['disclosure', { genre: 'deep house', weight: 1.0 }],
      ['diplo', { genre: 'electronic', weight: 0.9 }],
      ['zedd', { genre: 'electro house', weight: 1.0 }],
      ['kaskade', { genre: 'progressive house', weight: 1.0 }],
      ['steve aoki', { genre: 'electro house', weight: 1.0 }],
      ['afrojack', { genre: 'electro house', weight: 1.0 }],
      ['avicii', { genre: 'progressive house', weight: 1.0 }],
      ['swedish house mafia', { genre: 'progressive house', weight: 1.0 }],
      ['daft punk', { genre: 'electronic', weight: 1.0 }],
      ['justice', { genre: 'electronic', weight: 1.0 }],
  ...
  async enhanceEventWithArtistData(event, options = {}) {
    ...
    // NEW: Add genre detection FIRST (critical for confidence scoring)
    const genreEnhancedEvent = this.detectGenres(enhancedEvent);
    Object.assign(enhancedEvent, genreEnhancedEvent);
    ...
    if (event.artists && event.artists.length > 0) {
      ...
      for (const artist of event.artists) {
        // Normalize artist input into a safe string
        const artistName = this.normalizeArtistName(artist);
        if (!artistName) {
          console.warn('⚠️ Artist name is not a string after reconstruction, skipping:', typeof artist);
          continue;
        }

        // FIX: Create enhanced artist object properly (artist is a string)
        ...
        try {
          ...
          const profile = await this.artistProfileService.generateArtistProfile(artistName, {
          ...
          if (profile && profile.confidence > 0.3) {
            ...
            console.log(`🎵 Generated sound characteristics for ${artist}:`, {
              confidence: profile.confidence,
              source: profile.source,
              genreFamily: profile.genreFamily
            });
            ...
            const fallbackCharacteristics = this.getFallbackCharacteristics(artist, enhancedEvent.enhancedGenres);
          ...
          console.warn(`⚠️ Artist profiling failed for ${artist}:`, error.message);
  ...
  detectGenres(event) {
    ...
    if (event.artists && event.artists.length > 0) {
      for (const artistData of event.artists) {
        // Handle both corrupted objects and clean strings
        let artistName;
        
        if (typeof artistData === 'string') {
          artistName = artistData;
        } else if (artistData && typeof artistData === 'object') {
          // Handle corrupted artist objects
          if (artistData.name) {
            artistName = artistData.name; // Enhanced artist object
          } else if (artistData[0] && artistData[1]) {
            // Corrupted object - reconstruct the name
            const chars = Object.keys(artistData)
              .filter(key => !isNaN(key))
              .sort((a, b) => parseInt(a) - parseInt(b))
              .map(key => artistData[key]);
            artistName = chars.join('');
          } else {
        ...
        // Ensure artistName is a string before proceeding
        if (typeof artistName !== 'string') {
          console.warn('⚠️ Artist name is not a string after reconstruction, skipping:', typeof artistName);
          continue;
        }
  ...
  // NEW: Normalize artist input into a safe string or null
  normalizeArtistName(artistData) {
    if (!artistData) return null;

    // If it's already a string, trim and return
    if (typeof artistData === 'string') {
      const s = artistData.trim();
    ...
    if (typeof artistData === 'object') {
      if (artistData.name && typeof artistData.name === 'string') {
        const s = artistData.name.trim();
        return s.length > 0 ? s : null;
      }

      // If object uses numeric keys (corrupted array-like), reconstruct
      const numericKeys = Object.keys(artistData).filter(k => !isNaN(k));
      if (numericKeys.length > 0) {
        const chars = numericKeys
          .sort((a, b) => parseInt(a) - parseInt(b))
          .map(k => artistData[k]);
        const reconstructed = chars.join('');
        if (reconstructed && typeof reconstructed === 'string' && reconstructed.trim().length > 0) {
          return reconstructed.trim();
        }
      }

      // Fallback: try to stringify simple objects to get something usable
      try {
        const maybe = String(artistData).trim();
        return maybe.length > 0 ? maybe : null;
      } catch (e) {
        return null;
      }
    }
  ...
  extractArtistMetadata(event) {
    ...
    if (event.artists) {
      event.artists.forEach(artist => {
        const artistName = this.normalizeArtistName(artist);
        if (!artistName) return; // Skip invalid names
