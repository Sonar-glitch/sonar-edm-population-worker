  ...
  async enhanceEventWithArtistData(event, options = {}) {
    ...
    if (event.artists && event.artists.length > 0) {
      ...
      for (const artist of event.artists) {
        // FIX: Create enhanced artist object properly (artist is a string, not an object)
        const enhancedArtist = {
          name: artist, // artist is a string like "Don Diablo"
          originalName: artist
        };

        try {
          // Generate artist profile using the new service
          const profile = await this.artistProfileService.generateArtistProfile(artist, {
  ...
  extractArtistMetadata(event) {
    ...
    if (event.artists) {
      event.artists.forEach(artist => {
        ...
        if (!artistName || typeof artistName !== 'string') return; // Skip empty or non-string artist names
