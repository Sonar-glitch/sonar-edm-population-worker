...
class RecommendationEnhancer {
    ...
    try {
      ...
      return {
        ...
        enhancementMetadata: {
          ...
          version: '2.1', // Updated version for database fix
  ...
  // NEW: Clean event data for proper MongoDB serialization
  cleanEventForDatabase(event) {
    const cleaned = { ...event };

    // FIX: Ensure artists is always an array of strings
    if (cleaned.artists) {
      if (Array.isArray(cleaned.artists)) {
        // Convert any objects to strings, keep strings as-is
        ...
        cleaned.artists = [String(cleaned.artists)];
      }
    } else {
      cleaned.artists = [];
    }
    ...
    // FIX: Ensure artistList is always an array of strings (if present)
    if (cleaned.artistList) {
  ...
  extractArtists(event) {
    ...
    // Remove duplicates and validate - ENSURE STRINGS ONLY
    artists = [...new Set(artists)]
      .map(artist => String(artist)) // FIX: Ensure all are strings
      .filter(artist => typeof artist === 'string' && artist.length > 1 && artist.length < 50);

    return {
      ...event,
      artists, // FIX: This will always be an array of strings
      artistExtraction: {
  ...
  calculateEnhancedRecommendationScore(event) {
    ...
    return {
      ...
      recommendationMetrics: {
        ...
        version: '2.1', // Enhanced version with database fix
