// FINAL FIX: lib/recommendationEnhancer.js (OBJECT STRUCTURE PRESERVATION)
// Enhanced recommendation enhancement for events pipeline with new scoring factors
// Memory-optimized for Basic dyno (512MB)
// FIX: Preserve artist object structure for UnifiedEvent schema compatibility
...
class RecommendationEnhancer {
  ...
  async enhanceEvent(event) {
    ...
    try {
      ...
      // FIX: Ensure proper data serialization for MongoDB
      const cleanedEvent = this.cleanEventForDatabase(scoreEnhanced);
      ...
      return {
        ...
        enhancementMetadata: {
          ...
          version: '2.2', // Updated version for object structure fix
          stages: ['artist_extraction', 'genre_detection', 'enhanced_recommendation_scoring', 'data_cleaning']
        }
      };
    } catch (error) {
  ...
  // FIXED: Clean event data for proper MongoDB serialization while preserving object structure
  cleanEventForDatabase(event) {
    const cleaned = { ...event };

    // FIX: Preserve artist objects structure for UnifiedEvent schema
    if (cleaned.artists) {
      if (Array.isArray(cleaned.artists)) {
        // Ensure artists are objects with proper structure
        ...
        // If not an array, convert to array of objects
        cleaned.artists = [{
    ...
    // FIX: Ensure artistList is always an array of strings (for frontend)
    ...
    } else if (cleaned.artistList) {
      if (Array.isArray(cleaned.artistList)) {
        cleaned.artistList = cleaned.artistList.map(artist => String(artist)).filter(artist => artist && artist.length > 0);
      } else {
        cleaned.artistList = [String(cleaned.artistList)];
      }
    } else {
      cleaned.artistList = [];
    }

    // FIX: Ensure genres is always an array of strings
    if (cleaned.genres) {
      if (Array.isArray(cleaned.genres)) {
        cleaned.genres = cleaned.genres.map(genre => String(genre)).filter(genre => genre && genre.length > 0);
      } else {
    ...
    // FIX: Remove any circular references or complex objects that might cause serialization issues
    if (cleaned.artistExtraction && cleaned.artistExtraction.extractedAt) {
      cleaned.artistExtraction.extractedAt = new Date(cleaned.artistExtraction.extractedAt);
    }

    if (cleaned.genreDetection && cleaned.genreDetection.detectedAt) {
      cleaned.genreDetection.detectedAt = new Date(cleaned.genreDetection.detectedAt);
    }

    if (cleaned.recommendationMetrics && cleaned.recommendationMetrics.calculatedAt) {
      cleaned.recommendationMetrics.calculatedAt = new Date(cleaned.recommendationMetrics.calculatedAt);
    }

    if (cleaned.enhancementMetadata && cleaned.enhancementMetadata.processedAt) {
      cleaned.enhancementMetadata.processedAt = new Date(cleaned.enhancementMetadata.processedAt);
    }

    return cleaned;
  }

  // FIXED: Extract artists and preserve original object structure
  extractArtists(event) {
    ...
    if (event.artistList && Array.isArray(event.artistList)) {
      artistNames = [...event.artistList];
    }
    // PRESERVED: Extract from artists array (array of objects) - PRESERVE OBJECTS
    else if (event.artists && Array.isArray(event.artists)) {
      if (typeof event.artists[0] === 'object') {
        // Artists are objects - preserve them and extract names
        originalArtists = [...event.artists];
        artistNames = event.artists.map(a => a.name || a).filter(Boolean);
      } else {
        // Artists are strings - convert to objects
        artistNames = [...event.artists];
      }
    }
    // Extract from event name (last resort)
    else if (event.name) {
      artistNames = this.parseArtistsFromName(event.name);
    }

    // Remove duplicates and validate artist names
    artistNames = [...new Set(artistNames)]
      .map(artist => String(artist))
      .filter(artist => typeof artist === 'string' && artist.length > 1 && artist.length < 50);

    // If we have original artist objects, preserve them; otherwise create objects from names
    let finalArtists = [];
    if (originalArtists.length > 0) {
      // Preserve original objects but ensure they have proper structure
      finalArtists = originalArtists.map(artist => ({
        name: String(artist.name || ''),
        id: String(artist.id || ''),
        url: String(artist.url || ''),
        image: String(artist.image || ''),
      ...
      // Create objects from artist names
      finalArtists = artistNames.map(name => ({
        name: name,
        id: '',
        url: '',
        image: '',
    ...
    return {
      ...event,
      artists: finalArtists, // FIX: This will always be an array of objects
      artistList: artistNames, // FIX: This will always be an array of strings
      artistExtraction: {
        method: artistNames.length > 0 ? 'enhanced' : 'none',
        confidence: artistNames.length > 0 ? 80 : 0,
  ...
  // FIXED: Detect genres using artist names from objects
  detectGenres(event) {
    let genres = [];
    ...
    return {
      ...event,
      genres: [...new Set(genres)], // FIX: Ensure unique string array
      primaryGenre,
      isEdmEvent,
      edmConfidence,
      genreDetection: {
  ...
  calculateEnhancedRecommendationScore(event) {
    ...
    return {
      ...
      recommendationMetrics: {
        ...
        version: '2.2', // Enhanced version with object structure fix
        algorithm: 'enhanced_multi_factor'
      }
    };
  }

  // FIXED: Calculate genre score
  ...
  // FIXED: Calculate artist score using object structure
