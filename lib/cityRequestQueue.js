...
function getPendingCityRequests() {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const pendingRequests = await cityRequestsCollection.find({ 
        status: 'pending' 
      }).sort({ 
        priority: -1, 
        requestCount: -1, 
      ...
      // SURGICAL FIX: Always resolve with array, never undefined
      const safeArray = Array.isArray(pendingRequests) ? pendingRequests : [];
      resolve(safeArray);
      
    } catch (error) {
      console.warn('⚠️ Error fetching pending city requests:', error.message);
      // SURGICAL FIX: Resolve with empty array on error to prevent forEach crash
      resolve([]);
    }
  });
}
